...
export default function Create({ title, setCurrent, setBack, phone, setPhone, setFullName }) {
  ...
  return (
    <div className="create__wrap" >
      <div className="create__input">
        ...
        <input type="text" onChange={(e) => { setFullName(e.target.value) }} name="" id="" className='create__input-inp' />
      </div>
      <div className="create__input">
        <p className='create__input-text'>{t("tel")} <span>{t("codeSendViaTgBot")}</span></p>
        <Input phone={phone} setPhone={setPhone} />
      </div>
      <button className='create__btn' onClick={handleRegister}>{t("createAcc")}</button>
      <p className='create__text'>{t("haveAcc")} <span onClick={() => { setCurrent('login') }} className='create__link' >{t("login")}</span></p>
    </div>
