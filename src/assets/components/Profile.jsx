...
import ChooseCity from './tools/ChooseCity';
...
export default function Profile() {
    ...
    const [isEditingAddress, setIsEditingAddress] = useState(false); // Новое состояние для адреса
    ...
    const handleSave = async () => {
        ...
        let rawPhone = formData.phone.replace(/\s/g, ''); // Удаляем пробелы
        rawPhone = rawPhone.padStart(12, "998"); // ✅ Добавляем `998`, если отсутствует
        ...
        try {
            await axios.patch(
                `${API_BASE_URL}/auth/users/me/`,
                {
                    ...
                    username: rawPhone, // ✅ Теперь номер сохраняется корректно
                },
                {
                    headers: {
